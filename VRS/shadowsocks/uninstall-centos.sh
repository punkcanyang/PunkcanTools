...
main() {
    echo -e "${CYAN}╔══════════════════════════════════════════════╗"
    echo "║   Shadowsocks 卸载脚本 (CentOS/RHEL)          ║"
    echo -e "╚══════════════════════════════════════════════╝${NC}"
    [[ $EUID -ne 0 ]] && { echo -e "${RED}需要 root 权限${NC}"; exit 1; }
    echo -e "${YELLOW}警告: 将完全卸载 Xray (Shadowsocks)${NC}"
    ...
    bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ remove --purge 2>/dev/null || {
        rm -f /usr/local/bin/xray; rm -rf /usr/local/etc/xray; rm -f /etc/systemd/system/xray.service; systemctl daemon-reload
    }
    log_success "已卸载"
    echo -e "\n${GREEN}════════ 卸载完成！════════${NC}"
}
