...
CLIENT_CONFIG_FILE="/etc/trojan-go/client-config.txt"
SHARE_LINK_FILE="/etc/trojan-go/trojan-go-link.txt"

show_help() {
    echo -e "${CYAN}Trojan-Go 配置查看工具${NC}"
...
show_link() {
    [[ -f "$SHARE_LINK_FILE" ]] && cat "$SHARE_LINK_FILE" || grep "^trojan://" "$CLIENT_CONFIG_FILE" 2>/dev/null || echo -e "${RED}未找到链接${NC}"
}

show_qr() {
    if ! command -v qrencode &> /dev/null; then echo -e "${YELLOW}qrencode 未安装${NC}"; show_link; return; fi
    local link; [[ -f "$SHARE_LINK_FILE" ]] && link=$(cat "$SHARE_LINK_FILE") || link=$(grep "^trojan://" "$CLIENT_CONFIG_FILE" 2>/dev/null)
