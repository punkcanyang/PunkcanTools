...
# Resolve lib/ — works both from the repo (./lib) and the installed copy (/usr/local/share/vrs/lib).
...
VRS_LIB_DIR=$(_resolve_vrs_lib_dir) || {
    log_error "未找到 VRS 共享 lib 目录（尝试过 \$VRS_LIB_DIR / \$SCRIPT_DIR/lib / /usr/local/share/vrs/lib）"
    exit 1
}
...
# Read the listening port from the existing Xray config so we can drop firewall rules later.
