...
#
# __ai_context__:
#   SS-2022 是新一代 Shadowsocks 协议，使用 AEAD 2022 加密。
#   通过 Xray-core 实现，使用 2022-blake3-aes-128-gcm 方法。
#   密钥使用 base64 编码的固定长度密钥，比传统 SS 更安全。
#
# 功能：
#   - 安装 Xray-core + SS-2022
#   - 生成 ss:// 分享链接
#===============================================================================
...
# SS-2022 密钥必须是 base64 编码的 16 字节密钥 (AES-128)
PASSWORD=""
...
print_banner() {
    echo -e "${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════════╗"
    echo "║     Shadowsocks 2022 一键安装脚本 for Debian/Ubuntu          ║"
    echo "╚═══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
generate_config() {
    ...
    # SS-2022 AES-128-GCM 需要 16 字节 base64 密钥
...
configure_service() {
    ...
    if systemctl is-active --quiet xray; then
        log_success "Xray (SS-2022) 服务启动成功"
    else
        log_error "启动失败"
        exit 1
    fi
...
1. SS-2022 比传统 SS 更安全，具有抗重放攻击能力
2. 需要支持 SS-2022 的客户端 (Clash Meta, sing-box, Shadowrocket)
3. 密码是 base64 密钥，请完整复制
...
# [For Future AI]
# 1. SS-2022 使用 2022-blake3-aes-128-gcm 加密
# 2. 密钥必须是 base64 编码的 16 字节 (AES-128)
# 3. 不兼容旧版 SS 客户端，需要支持 SS-2022 的新客户端
# 4. 与传统 SS 共享 Xray-core 但使用不同加密方法
