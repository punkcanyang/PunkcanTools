#!/bin/bash
#===============================================================================
# VLESS + WebSocket + TLS 配置查看脚本
#===============================================================================
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[0;33m'; CYAN='\033[0;36m'; NC='\033[0m'

CLIENT_CONFIG_FILE="/usr/local/etc/xray/client-config.txt"
SHARE_LINK_FILE="/usr/local/etc/xray/vless-ws-link.txt"

show_help() {
    echo -e "${CYAN}VLESS + WebSocket + TLS 配置查看工具${NC}"
...
show_link() {
    [[ -f "$SHARE_LINK_FILE" ]] && cat "$SHARE_LINK_FILE" || grep "^vless://" "$CLIENT_CONFIG_FILE" 2>/dev/null || echo -e "${RED}未找到链接${NC}"
}

show_qr() {
    if ! command -v qrencode &> /dev/null; then
        echo -e "${YELLOW}qrencode 未安装${NC}"; show_link; return
    fi
    local link; [[ -f "$SHARE_LINK_FILE" ]] && link=$(cat "$SHARE_LINK_FILE") || link=$(grep "^vless://" "$CLIENT_CONFIG_FILE" 2>/dev/null)
