...
CLIENT_CONFIG_FILE="/etc/tuic/client-config.txt"
SHARE_LINK_FILE="/etc/tuic/tuic-link.txt"

show_help() {
    echo -e "${CYAN}TUIC v5 配置查看工具${NC}"
...
show_link() {
    [[ -f "$SHARE_LINK_FILE" ]] && cat "$SHARE_LINK_FILE" || grep "^tuic://" "$CLIENT_CONFIG_FILE" 2>/dev/null || echo -e "${RED}未找到链接${NC}"
}

show_qr() {
    if ! command -v qrencode &> /dev/null; then echo -e "${YELLOW}qrencode 未安装${NC}"; show_link; return; fi
    local link; [[ -f "$SHARE_LINK_FILE" ]] && link=$(cat "$SHARE_LINK_FILE") || link=$(grep "^tuic://" "$CLIENT_CONFIG_FILE" 2>/dev/null)
